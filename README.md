...
Auto complete component for Angular 7.

Click for [demo](http://oferh.github.io/fe-completer/)

## Installation

```sh
npm install fe-completer --save
```

## Usage

The module you want to use completer in must import `CompleterModule` and `FormsModule` (to use the ngModel
directive on fe-completer).  `CompleterModule` provides the `CompleterService`, and declares the `fe-completer`
directive.
```ts
...
import { CompleterModule } from 'src/shared/completer';
...
Add completer to your component and create a data source:

```ts
import { Component } from '@angular/core';
import { CompleterService, CompleterData } from 'src/shared/completer';

@Component({
  selector: 'my-component',
  template: `<h1>Search color</h1>
            <fe-completer [(ngModel)]="searchStr" [datasource]="dataService" [minSearchLength]="0"></fe-completer>
            <h1>Search captain</h1>
            <fe-completer [(ngModel)]="captain" [datasource]="captains" [minSearchLength]="0"></fe-completer>`
})
...
fe-completer uses [rxjs](https://github.com/Reactive-Extensions/RxJS) stream as data sources.
...
```ts
   var map = {
       ...
       'fe-completer': 'node_modules/fe-completer/fe-completer.umd.js'
   }
```
